import axios from "axios"



const api_url = process.env.NEXT_PUBLIC_API_URL;
const token = ''
export const HomeData = async(locale:String)=>{

    const response = await fetch(`${api_url}/api/home-page?populate[Hero][populate]=*&populate[Services][populate]=*&locale=${locale}`,{
        headers:{
            'Authorization': `Bearer ${token}`, // Add the token here
            
        }    
})}